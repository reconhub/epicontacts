---
title: "Visualisation of timed transmission trees using epicontacts"
date: "`r Sys.Date()`"
output: 
...
plot(sim,
     ...
     node_size = 'R_i',
     node_color = 'sex',
     col_pal = colorRampPalette(c("#ba1717", "#006b99")),
...
infectee in the `from` and `to` columns, respectively, as well as the contact
status (suspected or confirmed), duration of contact and date of first contact.
...
We can then visualise of these transmission chains using the `plot` function. We
specify that we want to visualise our network as a transmission tree using the
`method` argument, and indicate which column in our linelistp should be used for
the x-axis (in this case, the date of symptom onset).

```{r}
plot(sim, method = 'ttree', x_axis = 'onset')
```
...
Finally, the position of the parent node relative to the children nodes can be
...
## Time axis
A second date axis can be added at the top of the plot using the `double_axis` 
argument. The format of the date labels can be specified using the `date_labels` 
argument, which uses the standard codes defined in `strftime()`.
...
`ttree_shape = 'branching'` due to a bug within `visNetwork`. Try double
...
argument. Node labels can be hidden using the `hide_labels` argument. This can
...
contacts_2 <- data.frame(from = from, 
                        to = to,
...
sim_2 <- sim
sim_2$contacts <- rbind(sim_2$contacts, contacts_2)
sim_2 $directed <- TRUE
plot(sim_2, 
     method = 'ttree', 
     x_axis = 'onset',
