...
```{r, echo = FALSE}
set.seed(1)
to <- sample(x$linelist$id, 5)
from <- sapply(match(to, x$linelist$id), 
               function(i) sample(x$linelist$id[x$linelist$onset < x$linelist$onset[i]], 1))

contact_2 <- data.frame(from = from, 
                        to = to,
                        status = sample(c("confirmed", "suspected"), 5, TRUE),
                        duration = runif(5, 0, 20),
                        date = sample(x$linelist$onset, 5))

x$contacts <- rbind(x$contacts, contact_2)
```
```{r}
plot(x, 
     type = 'ttree', 
