...
set.seed(2)
sim_3 <- sim
sim_3$contacts$from <- sample(sim$contacts$from, replace = FALSE)
```
...
si <- get_pairwise(sim_3, 'onset', function(x, y) y - x)
sim_3$contacts$suspicious <- si < min_si | si > max_si
...
```{r, echo = TRUE}
sim_3_clean <- sim_3
sim_3_clean$contacts <- sim_3$contacts[!sim_3$contacts$suspicious,]
