...
test_that("Return errors / warnings when expected", {
 ...
 not.epi_contacts <- list("linelist" = ebola.sim$linelist,
                          "contacts" = ebola.sim$contacts,
                          "directed" = FALSE)
  ...
  n.attr <- list("gender" = "f",
                 "date.of.infection" = c("2014-04-08", "2015-03-28"))
  expect_error(subset.epi_contacts(x,
                                   node.attribute = n.attr,
                                   edge.attribute = list("source" = "funeral")),
               "date.of.infection must be provided as a date object")

  n.attr <- list("gender" = "f",
                 "date.of.infection" = as.Date(c("1990-04-08","1990-03-28")))
  e.attr <- list("source" = "funeral")
  err.msg <- paste0("Dates provided for date.of.infection ",
                    "fall outside the range of the dataset")
  expect_error(suppressWarnings(subset.epi_contacts(x,
                                   node.attribute = n.attr,
                                   edge.attribute = e.attr)),
                                   err.msg)

  n.attr <- list("gender" = "f",
                 "date.of.infection" = as.Date(c("2014-04-08",
                                                 "2014-04-08",
                                                 "2014-04-08")))
  e.attr <- list("source" = "funeral")
  msg <- paste0("More than two date values provided ",
                "for date.of.infection, using first two")
