...
#' Dates must be provided as a vector of date objects, defining the range of dates included in the subset. If only one date is provided, only node attributes with that date will be returned.
#'
...
#' Dates must be provided as a vector of date objects, defining the range of dates included in the subset. If only one date is provided, only edge attributes with that date will be returned.
#'
...
subset.epi_contacts <- function(x,node.attribute=NULL,edge.attribute=NULL,cluster_id=NULL,
    ...
    ## name.attribute is the name of the attribute being checked (e.g. "date.of.infection")
    ## list.attributes is the named list of attributes (either node.attribute or edge.attribute)
    ## dataset is the dataframe describing the attribute values in the data (x$linelist or x$contacts)
    check <- function(name.attribute,list.attributes,dataset){
        ...
        ## Attribute is a single node or edge attribute value (e.g. "f", if name.attribute is "gender")
        attribute <- list.attributes[[name.attribute]]

        ## Data is the set of node or attribute values found in the data (e.g. a vector of infection dates)
        ...
        if(inherits(data,"Date")){
            ...
            if(length(attribute)>2){
                ...
                warning(paste0("More than two date values provided for ",name.attribute,", using first two"))
            }
            if(attribute[1]>attribute[2]){
                attribute <- c(attribute[2],attribute[1])
                warning(paste("First date provided for",name.attribute,"is greater than second,
                               using second date as lower boundary"))
            }
            if(min(attribute)>max(data,na.rm=TRUE) | max(attribute)<min(data,na.rm=TRUE))
                stop(paste("Dates provided for",name.attribute,"fall outside the range of the dataset"))
    ...
    ## A function to subset a dataset (x$linelist or x$contacts) by a node or edge attribute
    subs <- function(name.attribute,list.attributes,dataset){
        ...
        if(inherits(attribute,"Date"))
            dataset <- dplyr::filter(dataset,data >= attribute[1] & data <= attribute[2])
        else
    ...
    ## Check if epi_contacts object, node.attribute and edge.attribute are provided correctly
    ...
    node.attribute <- sapply(names(node.attribute),check,node.attribute,x$linelist,USE.NAMES=TRUE,simplify=FALSE)
    edge.attribute <- sapply(names(edge.attribute),check,edge.attribute,x$contacts,USE.NAMES=TRUE,simplify=FALSE)

    ## Apply the subs function across all attributes provided in node.attribute
    if(!(is.null(node.attribute)))
        for(name.attribute in names(node.attribute)) x$linelist <- subs(name.attribute,node.attribute,x$linelist)

    ## Apply the subs function across all attributes provided in edge.attribute
    if(!(is.null(edge.attribute)))
        for(name.attribute in names(edge.attribute)) x$contacts <- subs(name.attribute,edge.attribute,x$contacts)
