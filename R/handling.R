...
#'     contacts; defaults to the same as \code{i}
#'
#' @param contacts a character string indicating on which basis contacts are
#'     retained (see details)
#'
...
"[.epi_contacts" <- function(x, i, j=i, contacts=c("both","either","from","to"),
    ...
    ## contacts. The variable 'strict' triggers the subsetting of contacts; if
    ## TRUE, then both nodes need to be part of 'j' for a contact to be
    ## retained; if FALSE, only one of them needs to be in 'j'.

    ## 'k' and 'l' will be used to subset the columns of attributes in
    ## x$linelist and x$contacts, respectively; any usual subsetting is fine for
    ## these ones (index, logical, name), although the 'id', 'from' and 'to'
    ## columns are discarded from the subsetting. That is:
